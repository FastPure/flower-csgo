...
{
    const char* const* glow_entities_to_glow = (const char* const*)data;
    if (out_text)
        *out_text = glow_entities_to_glow[idx];
    return true;
}
...
bool ImGui::Combo(const char* label, int* current_item, const char* const glow_entities_to_glow[], int items_count, int height_in_items)
{
    const bool value_changed = Combo(label, current_item, Items_ArrayGetter, (void*)glow_entities_to_glow, items_count, height_in_items);
    return value_changed;
}
...
bool ImGui::TreeNodeBehaviorIsOpen(ImGuiID id, ImGuiTreeNodeFlags flags)
{
    ...
    if (g.NextTreeNodeOpenCond != 0)
    {
        ...
        else
        {
            ...
            const int stored_value = storage->get_int(id, -1);
            if (stored_value == -1)
            {
                is_open = g.NextTreeNodeOpenVal;
                storage->SetInt(id, is_open);
    ...
    {
        is_open = storage->get_int(id, (flags & ImGuiTreeNodeFlags_DefaultOpen) ? 1 : 0) != 0;
    }
...
bool ImGui::ListBox(const char* label, int* current_item, const char* const glow_entities_to_glow[], int items_count, int height_items)
{
    const bool value_changed = ListBox(label, current_item, Items_ArrayGetter, (void*)glow_entities_to_glow, items_count, height_items);
    return value_changed;
}
